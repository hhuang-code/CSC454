...
(* Check whether an id has been declared *)
let rec check_declared id id_dec_list =
    match id_dec_list with
    | [] -> false
    | h :: t -> if h = id then true else check_declared id t;;

(* Check whether an id has been used *)
let rec check_used id id_used_list =
    match id_used_list with
    | [] -> false
    | h :: t -> if h = id then true else check_used id t;;

(* Find all elements in l1 but not in l2, the return is also a list *)
let diff l1 l2 = 
    List.filter (fun x -> not (List.mem x l2)) l1

(* Return true if s contains sub, otherwise return false.
   Both s and sub are strings.
*)
let contains s sub =
...
let rec translate (ast : ast_sl) (program : string)
    ...
    (* The return value is a tuple, (declared variables list * used varialbes list * C program) *)
    ...
    (* 
      ...
      The first element of the returned tuple is a list containing all declared variables.
      The second element of the returned tuple is a list containing all used variables.
    *)
    ...
    (* 
      The elements that appear in the first list but not in the second list 
    ...
    *)
    ...
    (* Rearrange all unused variables from a list to human readable string *)
...
let () =   
    ...
    (* If the program contains unused variables, print warning messages *)
